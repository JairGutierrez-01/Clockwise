...
function showCustomModal({
  ...
  return new Promise((resolve) => {
    ...
    const handleKeydownDefault = (e) => {
      if (e.key === "Enter") {
        ...
        if (customModalConfirmBtn.style.display !== "none") {
          // Only trigger if confirm button is visible
        ...
        // Only trigger if cancel button is visible
        e.preventDefault();
        handleCancelDefault();
      }
    };
    ...
    if (customContentHtml) {
      ...
      setTimeout(() => {
        const firstFocusable = dynamicContentArea.querySelector(
...
function hideCustomModal() {
  ...
  if (dynamicContentArea) {
    // Clear dynamic content when hiding
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    teamsTableBody.addEventListener("click", async (event) => {
      ...
      try {
        ...
        if (!result.projects || !Array.isArray(result.projects)) {
          ...
          if (dynamicContentArea) {
            dynamicContentArea.querySelector(".modal-content-area").innerHTML =
              "<p>Error loading projects.</p>";
          }
          return;
        }

        const allTeamProjects = result.projects.filter(
  ...
  if (createTeamBtn) {
    createTeamBtn.addEventListener("click", async () => {
      ...
      const newTeamDescriptionInput =
        document.getElementById("newTeamDescription");
        ...
        try {
          ...
          if (response.ok) {
            if (data.error) {
              showCustomAlert(
                "Error",
                "Error adding member: " + data.error,
                "error",
              );
            } else {
              showCustomAlert(
  ...
  try {
    ...
    if (response.ok) {
      if (data.error) {
        showCustomAlert(
          "Error",
          "Error removing member: " + data.error,
          "error",
        );
      } else {
        ...
        try {
          ...
          if (!membersResponse.ok) {
            console.error(
              ...
              membersData.error || "Unknown error",
            );
            return { ...team, members: [] }; // Return team with empty members on backend error
          }
          if (!Array.isArray(membersData)) {
            console.error(
              `Unexpected members data format for team ${team.team_id}:`,
              membersData,
            );
              ...
              if (
                ...
                try {
                  ...
                  if (userDetailsResponse.ok && userDetails.username) {
                    ...
                    console.warn(
                      `Could not fetch username for user ID: ${member.user_id}. Response:`,
                      userDetails,
                    );
                  }
                } catch (userErr) {
...
`,
  ...
  document.addEventListener("click", async function (event) {
    ...
    if (event.target.classList.contains("open-assign-modal-btn")) {
      ...
      try {
        ...
        for (const project of teamProjects) {
          const res = await fetch(
            `/api/tasks?project_id=${project.project_id}`,
          );
          if (!res.ok) continue;
        ...
        const availableTasks = allTasks.filter(
          (task) => task.member_id === null || task.member_id === undefined,
        );

        if (availableTasks.length === 0) {
          showCustomAlert(
            "Info",
            "No available tasks to assign for this user.",
            "alert",
          );
          return;
        }

        const taskListHtml = availableTasks
          .map(
            (task) => `
        <li>
      ...
      `;
        ...
        dynamicContentArea
          .querySelectorAll(".assign-task-btn")
          .forEach((btn) => {
              ...
              try {
                ...
                if (assignResponse.ok) {
                  ...
                  const memberOptionsButton = document.querySelector(
                    `.assign-options-btn[data-user-id="${targetUserId}"]`,
                  );
                ...
                showCustomAlert(
                  "Error",
                  "Network error. Could not assign task.",
                  "error",
                );
              }
            });
    ...
    if (event.target.classList.contains("view-assigned-tasks-btn")) {
      ...
      try {
        ...
        if (!assignedTasks || assignedTasks.length === 0) {
          showCustomAlert(
            "Info",
            "No tasks currently assigned to this user in this project.",
            "alert",
          );
          return;
        }
  ...
  async function fetchAssignedTaskForUser(userId, teamId) {
    ...
    if (!response.ok) {
      console.error(
        "Error fetching user's tasks for userId:",
        userId,
        "Status:",
        response.status,
      );
