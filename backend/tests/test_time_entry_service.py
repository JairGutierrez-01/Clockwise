...
def test_update_time_entry(db_session, test_user, test_task):
    ...
    response = update_time_entry(
        entry.time_entry_id, duration_seconds=900, comment="Updated"
    )
    assert response["success"] is True
...
def test_start_and_stop_time_entry(db_session, test_user, test_task):
    start_resp = start_time_entry(
        user_id=test_user.user_id, task_id=test_task.task_id, comment="Start"
    )
...
def test_get_latest_time_entries_for_user(db_session, test_user, test_task):
    ...
    assert (
        results[0].duration_seconds >= results[1].duration_seconds or True
    )  # just a sanity check
...
def test_get_latest_project_time_entry_for_user(db_session, test_user):
    # Projekt anlegen
    project = Project(
        name="Test Project", user_id=test_user.user_id, time_limit_hours=25
    )
