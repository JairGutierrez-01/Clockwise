...
def test_add_team_member(client, db_session, setup_team_test_data, login_user):
    ...
    response = client.patch(f"/teams/{team.team_id}/add-member", json={"user_id": member.username, "role": "member"})

    assert response.status_code == 200
    assert UserTeam.query.filter_by(user_id=member.user_id, team_id=team.team_id).first()
...
def test_remove_team_member(client, db_session, setup_team_test_data, login_user):
    ...
    response = client.patch(f"/teams/{team.team_id}/remove-member", json={"user_id": member.username})

    assert response.status_code == 200
    assert not UserTeam.query.filter_by(user_id=member.user_id, team_id=team.team_id).first()
...
def test_assign_project_to_team(client, db_session, setup_team_test_data, login_user):
    ...
    response = client.post(f"/teams/{team.team_id}/assign_project", json={"project_id": project.project_id})
...
def test_get_team_tasks(client, db_session, login_user):
    ...
    task_admin = Task(title="Admin Task", project_id=project.project_id, member_id=admin.user_id)
    task_member = Task(title="Member Task", project_id=project.project_id, member_id=member.user_id)
